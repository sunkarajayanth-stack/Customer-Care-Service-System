...
If the page looks like plain HTML (no colors/cards), it means CSS/JS paths are not being resolved from your current location.

Use one of these:
```bash
cd /workspace/Customer-Care-Service-System
python3 -m http.server 8080
```
Then open:
- `http://localhost:8080/` (auto-redirects to the project), or
- `http://localhost:8080/customer-service-dsa-fwd/index.html`

Do **not** move individual HTML files without their `css/` and `js/` folders.
